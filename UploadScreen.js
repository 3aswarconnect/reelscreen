...
const UploadScreen = () => {
  ...
  const compressImage = async (imageFile) => {
    ...
    try {
      ...
      // Compress the image with high compression
      const manipResult = await ImageManipulator.manipulateAsync(
        imageFile.uri,
        [], // no transformations like resize
        {
          compress: 0.2, // High compression (0.2 = 20% quality)
      ...
      const compressedFile = {
        uri: manipResult.uri,
        name: imageFile.name.replace(/\.[^/.]+$/, ".jpg"), // Change extension to jpg
        mimeType: 'image/jpeg',
        size: compressedInfo.size
      };
      
      setCompressionStatus(`Compressed: ${(compressedInfo.size / fileInfo.size * 100).toFixed(1)}% of original`);
  ...
  const pickFile = async () => {
    ...
    if (!result.canceled) {
      ...
      // If it's an image, compress it immediately
      if (selectedFile.mimeType.includes('image')) {
        const compressedFile = await compressImage(selectedFile);
        setFile(compressedFile);
      } else {
        setCompressionStatus('No compression for video files');
  ...
  const uploadToServer = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.206.183:4000/upload', formData, {
              ...
              <View style={styles.compressionInfo}>
                ...
                {originalSize && compressedSize && (
                  <View style={styles.sizeInfoContainer}>
                    <Text style={styles.sizeInfo}>Original: {(originalSize / (1024 * 1024)).toFixed(2)} MB</Text>
                    <Text style={styles.sizeInfo}>Compressed: {(compressedSize / (1024 * 1024)).toFixed(2)} MB</Text>
                    <Text style={styles.sizeInfoSaved}>
